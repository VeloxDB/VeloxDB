	...
	internal sealed class FromDTOMethod : Method
	{
		...
		private MethodDesc CreateAdditionalMethodDesc()
		{
			return new MethodDesc()
			{
				...
				{
					new ParamDesc("VeloxDB.ObjectInterface.ObjectModel", "om"),
		...
		{
			string funcType = $"System.Func<VeloxDB.ObjectInterface.ObjectModel, {DTOType.ToString()}, VeloxDB.Common.AutomapperThreadContext, int, {Type.FullName}>";
		...
		private void GenerateCallInternalMethod(Context context, SourceWriter writer)
		{
			...
			{
				writer.AppendIndent();
				writer.Append("VeloxDB.Common.AutomapperThreadContext.Instance.AllowUpdate = ");
